...
/** @type {string} */
const CACHE_VERSION = '1.1-final-rc|7ed55d2e4460441775da';
/** @type {string} */
const CACHE_PREFIX = 'Celestial Ascent-sw-cache-';
const CACHE_NAME = CACHE_PREFIX + CACHE_VERSION;
/** @type {string} */
const OFFLINE_URL = 'celestial_ascent_1_1.offline.html';
/** @type {boolean} */
const ENSURE_CROSSORIGIN_ISOLATION_HEADERS = true;
// Files that will be cached on load.
/** @type {string[]} */
const CACHED_FILES = ["index.html", "celestial_ascent_1_1.144x144.png", "celestial_ascent_1_1.180x180.png", "celestial_ascent_1_1.512x512.png", "celestial_ascent_1_1.apple-touch-icon.png", "celestial_ascent_1_1.audio.position.worklet.js", "celestial_ascent_1_1.audio.worklet.js", "celestial_ascent_1_1.icon.png", "celestial_ascent_1_1.js", "celestial_ascent_1_1.manifest.json", "celestial_ascent_1_1.offline.html", "celestial_ascent_1_1.pck", "celestial_ascent_1_1.png", "celestial_ascent_1_1.wasm"];
...
self.addEventListener('activate', (event) => {
	event.waitUntil(caches.keys().then(
		function (keys) {
			// Remove old caches.
			return Promise.all(keys.filter((key) => (key.startsWith(CACHE_PREFIX) || key.startsWith('celestial_ascent_1_0-sw-cache-') || key.startsWith('celestial_ascent_1_1-sw-cache-')) && key !== CACHE_NAME).map((key) => caches.delete(key)));
		}
	).then(function () {
	...
	(event) => {
		...
		if (isNavigate || isCacheable) {
			event.respondWith((async () => {
				...
				if (cached == null && isNavigate && new URL(event.request.url).pathname === new URL(self.registration.scope).pathname) {
				...
				try {
					...
					if (isNavigate) return await cache.match(OFFLINE_URL);
